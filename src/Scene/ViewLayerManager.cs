...
namespace Game.Scene {
    ...
    /// - All async UniTask methods carry this.destroyCancellationToken
    ...
    public class ViewLayerManager : MonoBehaviour {
        ...
        public void RequestEnterCockpit(string shipId) {
            ...
            _ = SWITCHING_IN_SequenceAsync(shipId, this.destroyCancellationToken);
        }

        /// <summary>Request to return to star map from cockpit. Triggers SWITCHING_OUT.</summary>
        public void RequestReturnToStarMap() {
            if (_isSwitching) return;
            _ = SWITCHING_OUT_SequenceAsync(this.destroyCancellationToken);
        }
        ...
        public void RequestSwitchShip(string targetShipId) {
            ...
            _ = SWITCHING_SHIP_SequenceAsync(targetShipId, this.destroyCancellationToken);
        }
        ...
        public void RequestOpenOverlay() {
            if (_isSwitching) return;
            _ = OPENING_OVERLAY_SequenceAsync(this.destroyCancellationToken);
        }

        /// <summary>Request to close star map overlay. Triggers CLOSING_OVERLAY.</summary>
        public void RequestCloseOverlay() {
            if (_isSwitching) return;
            _ = CLOSING_OVERLAY_SequenceAsync(this.destroyCancellationToken);
        }
        ...
        private async UniTask SWITCHING_IN_SequenceAsync(string shipId, CancellationToken ct) {
            _isSwitching = true;
            RecordStarMapCameraPosition();

            try {
                // Step 2: cache pre-enter state
                var shipData = ShipDataModel.GetShipData(shipId);
        ...
        private async UniTask SWITCHING_OUT_SequenceAsync(CancellationToken ct) {
            _isSwitching = true;
            ...
            try {
                ...
                var shipData = ShipDataModel.GetShipData(_activeShipId);
                if (shipData != null) {
                ...
                if (unloadOp != null) {
                    await UniTask.WaitUntil(() => unloadOp.isDone, cancellationToken: ct);
                }
                if (_transitionMask != null) {
                    await _transitionMask.FadeOutAsync(0.3f, ct);
                }
        ...
        private async UniTask SWITCHING_SHIP_SequenceAsync(string targetShipId, CancellationToken ct) {
            _isSwitching = true;
            ...
            try {
                ...
                var oldShipData = ShipDataModel.GetShipData(_activeShipId);
                ...
                var targetShipData = ShipDataModel.GetShipData(targetShipId);
        ...
        private async UniTask OPENING_OVERLAY_SequenceAsync(CancellationToken ct) {
            _isSwitching = true;
        ...
        private async UniTask CLOSING_OVERLAY_SequenceAsync(CancellationToken ct) {
            _isSwitching = true;
        ...
        // ─── Helpers ───────────────────────────────────────────────────────────
        private async UniTask WaitForSceneLoadProgressAsync(
