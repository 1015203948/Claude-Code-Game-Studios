...
namespace Gameplay.Combat
{
    ...
    public class HitboxDetector
    {
        ...
        // 内部状态
        private readonly List<RaycastHit> _hitBuffer = new List<RaycastHit>(16);
        ...
        public IReadOnlyList<HitResult> Cast(Vector3 origin, Vector3 direction, float radius, float distance, float timestamp)
        {
            ...
            {
                // 按距离排序（由近到远）
                _hitBuffer.Sort((a, b) => a.distance.CompareTo(b.distance));

                // 限制最大命中数
                int count = Mathf.Min(_hitBuffer.Count, _maxHitsPerCast);
