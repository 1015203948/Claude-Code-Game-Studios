...
namespace Game.Tests.Integration.Gameplay {
    ...
    private sealed class MockShipStateChannel : ShipStateChannel {
        ...
        public override void Subscribe(Action<string, ShipState> handler) {
            OnShipStateChanged += handler;
        }

        public override void Unsubscribe(Action<string, ShipState> handler) {
            OnShipStateChanged -= handler;
        }
    ...
    private sealed class MockDualJoystickInput : DualJoystickInput {
