...
public class PhysicsCore_Test
{
    ...
    public void SetUp()
    {
        // Clean up any scene-resident ShipControlSystem that would hijack Instance
        ...
        // Ensure _rb is set — Awake's singleton check may destroy the GO
        // if a prior test left Instance non-null across fixture boundaries
        SetField(_scs, "_rb", _rb);
    ...
    {
        var method = typeof(ShipControlSystem).GetMethod("FixedUpdate",
            BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly);
        Assert.IsNotNull(method, "FixedUpdate method not found on ShipControlSystem");
    ...
    {
        var method = typeof(ShipControlSystem).GetMethod("LateUpdate",
            BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly);
        Assert.IsNotNull(method, "LateUpdate method not found on ShipControlSystem");
        method.Invoke(_scs, null);
    }
    ...
    public void p1_thrust_applies_addforce_forward()
    {
        ...
        InvokeFixedUpdate();
        Assert.Greater(_rb.linearVelocity.magnitude, 0f, "Thrust should increase velocity");
    }
    ...
    public void p1_thrust_respects_magnitude()
    {
        ...
        InvokeFixedUpdate();
        Assert.Greater(_rb.linearVelocity.magnitude, 0f);
    }
    ...
    {
        _rb.linearVelocity = new Vector3(3f, 0f, 3f); // ~4.24 m/s
        SetInput(Vector2.zero, Vector2.zero);
    ...
    {
        _rb.linearVelocity = new Vector3(10f, 0f, 10f); // ~14.14 m/s > 12
        SetInput(Vector2.zero, Vector2.zero);
        InvokeFixedUpdate();
    ...
    public void p3_thrust_changes_velocity_via_addforce_only()
    {
        ...
        InvokeFixedUpdate();
        Assert.Greater(_rb.linearVelocity.magnitude, 0f);
        Assert.AreNotEqual(15f, _rb.linearVelocity.magnitude);
    }
    ...
    {
        _rb.linearVelocity = new Vector3(3f, 2f, 4f);
        InvokeLateUpdate();
        Assert.AreEqual(0f, _rb.linearVelocity.y, 0.001f);
        Assert.AreEqual(3f, _rb.linearVelocity.x, 0.001f);
        Assert.AreEqual(4f, _rb.linearVelocity.z, 0.001f);
    }

    [Test]
    public void p6_pure_vertical_velocity_becomes_zero()
    {
        _rb.linearVelocity = new Vector3(0f, 5f, 0f);
        InvokeLateUpdate();
        Assert.AreEqual(0f, _rb.linearVelocity.magnitude, 0.001f);
    }
    ...
    public void integration_thrust_and_turn_same_frame()
    {
        ...
        InvokeFixedUpdate();
        Assert.Greater(_rb.linearVelocity.magnitude, 0f, "Thrust should work");
        Assert.AreEqual(Vector3.zero, _rb.angularVelocity,
